...
import_toshiba_data; % Generated by MATLAB automatically using GUI import

% Convert timestamps and extract relevant WattsUp data for Toshiba
toshiba_trace_wallwart_off = NaN(size(toshibachargedata,1),2); % Time in seconds and power in Watts
%toshibachargedata(:,1) = toshibachargedata(:,1) - toshibachargedata(1,1); % Normalize timestamps
%toshibachargedata(:,1) = toshibachargedata(:,1) * 21.9298 * 3600; % Convert timestamps to seconds
%toshiba_trace_wallwart_off(:,1) = toshibachargedata(:,1); % Copy timestamps
toshiba_trace_wallwart_off(:,1) = [0:10:(size(toshibachargedata,1)-1)*10]'; % Time in sec
toshiba_trace_wallwart_off(:,2) = toshibachargedata(:,2); % Copy power

toshiba_trace_wallwart_prime95 = NaN(size(toshibachargedatarunningprime95,1),2); % Time in seconds and power in Watts
%toshibachargedatarunningprime95(:,1) = toshibachargedatarunningprime95(:,1) - toshibachargedatarunningprime95(1,1); % Normalize timestamps
%toshibachargedatarunningprime95(:,1) = toshibachargedatarunningprime95(:,1) * 21.9298 * 3600; % Convert timestamps to seconds
%toshiba_trace_wallwart_prime95(:,1) = toshibachargedatarunningprime95(:,1); % Copy timestamps
toshiba_trace_wallwart_prime95(:,1) = [0:10:(size(toshibachargedatarunningprime95,1)-1)*10]'; % Time in sec
toshiba_trace_wallwart_prime95(:,2) = toshibachargedatarunningprime95(:,2); % Copy power

%% Output pre-processed Toshiba traces as CSV
csvwrite([toshiba_pre_processed_data_path 'toshiba_trace_wallwart_off.csv'], toshiba_trace_wallwart_off);
csvwrite([toshiba_pre_processed_data_path 'toshiba_trace_wallwart_prime95.csv'], toshiba_trace_wallwart_prime95);
...
clear toshibachargedata;
clear toshibachargedatarunningprime95;
